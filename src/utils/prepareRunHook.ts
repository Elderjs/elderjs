import { THookName } from '../hooks/types.js';
import { Elder, Page } from '../index.js';
...
function prepareRunHook({ hooks, hookInterface, settings }) {
  let internalHookInterface = hookInterface;
  let internalHooks = hooks;
  return {
    updateRunHookHookInterface: (newHookInterface) => (internalHookInterface = newHookInterface),
    updateRunHookHooks: (newHooks) => (internalHooks = newHooks),
    runHook: async function runHook(hookName: THookName, props: Elder | Page) {
      if (props.perf) props.perf.start(`hook.${hookName}`);

      // do we have a contract for the hook
      const hookDefinition = internalHookInterface.find((h) => h.hook === hookName);
      if (!hookDefinition) {
      ...
      const theseHooks = internalHooks.filter((h) => h.hook === hookName);
