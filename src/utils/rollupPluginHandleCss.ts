...
export default function elderjsHandleCss() {
  const isCss = (id) => id.includes('.css');
  const isSvelte = (id) => id.includes('.svelte');
  ...
  return {
    ...
    renderChunk(code, chunk, options) {
      if (chunk.isEntry) {
        ...
        const requiredCss = new Set(Object.keys(chunk.modules).map((c) => c.replace('.svelte', '.css')));

        const { css, map } = [...requiredCss].reduce(
          (out, key) => {
            const cssKey = key.replace('.svelte', '.css');
            if (cssMap.has(cssKey)) {
              const [thisCss, thisMap] = cssMap.get(cssKey).split('\n');
        ...
        code += `\nmodule.exports._css = \`${JSON.stringify(css)}\``;
        code += `\nmodule.exports._cssMap = \`${JSON.stringify(map)}\``;
        return code;
      }
      return null;
    },
    ...
    /* Kind: async, sequential                                                                                          */
    //     generateBundle(options, bundle: Object, isWrite) {}

    /* Type: (bundle: { [fileName: string]: AssetInfo | ChunkInfo }) => void */
    /* Kind: async, parallel                                                  */
    writeBundle(bundle) {
      const { css, map } = [...cssMap].reduce(
        (out, [path, code]) => {
          const [thisCss, thisMap] = code.split('\n');
          out.css.push(thisCss);
          out.map.push(thisMap);
          return out;
        },
