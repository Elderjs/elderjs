...
async function bootstrapPlugin(elder, pluginName) {
  ...
  return { pluginRoutes, pluginHooks, pluginShortcodes, pluginName };
}
...
async function plugins(elder: Elder) {
  ...
  const ps = pluginNames.map((pluginName) => bootstrapPlugin(elder, pluginName));

  const results = await Promise.all(ps);

  for (const plugin of results) {
    if (!plugin) continue;
    const { pluginRoutes: pRoutes, pluginHooks: pHooks, pluginShortcodes: pShortcodes } = plugin;
    pluginRoutes = { ...pluginRoutes, ...pRoutes };
    pluginHooks.push(...pHooks);
    pluginShortcodes.push(...pShortcodes);
  }
