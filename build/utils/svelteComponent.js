...
const svelteComponent = (componentName) => ({ page, props, hydrate = 0 }) => {
    ...
    const ssrCompontent = path_1.default.resolve(process.cwd(), `./${page.settings.locations.svelte.ssrComponents}${cleanComponentName}.js`);
    let clientSvelteFolder = page.settings.locations.svelte.clientComponents.replace(page.settings.locations.public, '/');
    if (clientSvelteFolder.indexOf('.') === 0)
        clientSvelteFolder = clientSvelteFolder.substring(1);
    ...
    try {
        ...
        function init${cleanComponentName.toLowerCase()}${id}() {
          System.import('${clientComponent}').then(({ default: App }) => {
            new App({ target: document.getElementById('${cleanComponentName.toLowerCase()}-${id}'), hydrate: true, props: ${JSON.stringify(props)} });
          });
        }
        ${IntersectionObserver_1.default({
      ...
      `,
        ...
        const matches = htmlOutput.matchAll(/<div class="needs-hydration" data-component="([A-Za-z]+)" data-data="({.*})"><\/div>/gim);
        for (const match of matches) {
            ...
            if (hydrate > 1) {
                throw new Error(`Client side hydrated component includes client side hydrated sub component. This isn't supported.`);
            }
